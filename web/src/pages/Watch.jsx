...
export default function Watch(){
  ...
  async function requestFullscreen(){
    // We emulate fullscreen ("theater mode") with CSS instead of native fullscreen.
    // Native fullscreen on iOS/Android hides our overlays (score/result + exit button).
    setTheater(true);
    // Prevent background scroll while in theater mode.
    document.body.style.overflow = "hidden";

    // Ensure the video starts playing (user gesture already happened).
  ...
  function closeTheater(){
    ...
    // No scrollIntoView here (it caused layout jumps on exit).
  }
  ...
  // Auto-retry if stuck on "Verbinde..." for too long (host restarted / race condition)
  useEffect(()=>{
    if(!note || !note.toLowerCase().includes('verbinde')) return
    const t = setTimeout(()=>{
      if(!playReady){
        try{ pcRef.current?.close() }catch{}
        pcRef.current = null
  ...
  async function acceptOffer(sdp, hostId){
    ...
    setNote('Verbinde...')
    setPlayReady(false)
    // Reset previous connection/stream
    try{ pcRef.current?.close() }catch{}
    pcRef.current = null
    ...
    pc.oniceconnectionstatechange = ()=>{
      ...
      if (st === 'disconnected' || st === 'failed') {
        // Auto-retry: ask host for a fresh offer
        setTimeout(()=>{ try{ sig.send({ type:'viewer-join', code }) }catch{} }, 800)
      }
    }
  ...
  async function ensurePlayback(){
    ...
    if(!v) return
    try{
      // Keep current mute state; browsers often block autoplay with audio.
      v.muted = muted
      await v.play?.()
      setPlayReady(true)
      setNote('')
  ...
  async function handlePlay(){
    // Always allow user to retry join; fixes "Verbinde..." dead state.
    try{
    ...
    <div className="card">
      ...
      <div ref={containerRef} className={"video" + (theater ? " theaterOn" : "")} style={{position:'relative'}}>
        ...
        <div style={{position:'absolute',top:10,right:10,display:'flex',gap:8,zIndex:6}}>
          ...
          <button className="fsBtn" onClick={()=> (theater ? closeTheater() : requestFullscreen())} aria-label="Fullscreen">
            ⤢
          </button>
        </div>
